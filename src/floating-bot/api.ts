...
//     ? ["/query"]  // Use Vite proxy in development (proxy targets VITE_TEXT2SQL_API_URL)
...
// Base URL for REST API endpoints (Factory, DC, Store KPIs, etc.)
export const API_BASE_URL = TEXT2SQL_API_URL;
...
// -------------------------------------------------------------------
// Text2SQL API call (SINGLE SOURCE OF TRUTH)
// -------------------------------------------------------------------
export const sendQuery = async (question: string): Promise<QueryResponse> => {
  if (!TEXT2SQL_API_URL) {
    throw new Error("VITE_TEXT2SQL_API_URL is not defined in environment");
  }

  const endpoint = `${TEXT2SQL_API_URL}/query`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.error ||
        errorData.details ||
        `Text2SQL API error: ${response.status}`
    );
  }
