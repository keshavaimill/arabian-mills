...
export const ChatWindow = ({
  ...
  return (
    <div
      className={`bg-card border-2 border-primary/20 rounded-xl shadow-2xl flex flex-col overflow-hidden ${
        isOpen ? "opacity-100" : "opacity-0 pointer-events-none"
      } ${
        isFloating
          ? `fixed right-4 z-50 ${
              isExpanded
                ? "bottom-4 w-[95vw] h-[90vh]"
                : "bottom-24 w-96 h-[600px]"
            }`
          : "w-full max-w-3xl mx-auto h-[700px] mt-6"
      }`}
    >
      {/* HEADER */}
      <div className="flex items-center justify-between p-4 border-b bg-muted/30">
        <div className="flex items-center gap-3">
          <img src="/image.png" className="w-10 h-10 rounded-full" />
          <div>
        ...
        <div className="flex gap-1">
          ...
          <Button variant="ghost" size="icon" onClick={handleOpenFullPage}>
            <ExternalLink className="w-4 h-4" />
          </Button>
          <Button
            variant="ghost"
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {/* ✅ RENDER BACKEND PLOT */}
              {msg.viz && msg.mime?.startsWith("image/") && (
                <div className="mt-3 rounded-lg overflow-hidden border bg-white">
                  <img
                    src={`data:${msg.mime};base64,${msg.viz}`}
                    alt="Generated chart"
