from src.Text2SQL_V2.utils.llm_factory import load_llm
...
class Text2SQLAgent:
    ...
    - If a column or table is NOT in the schema, it DOES NOT EXIST
    - If the user asks about data not in the schema, state it is not available and still return valid SQL when possible
    ...
    READ-ONLY RULE (VERY IMPORTANT):
    - This system is STRICTLY READ-ONLY
    - NEVER generate INSERT, UPDATE, DELETE, DROP, ALTER, or CREATE statements
    - ONLY SELECT queries are allowed

    FILTERING RULES:
    - For text columns, use LOWER(column) LIKE '%value%' unless the user explicitly asks for an exact match
    - For dates, use standard SQLite date comparisons

    AGGREGATION RULES:
    - Use SUM for quantity or stock totals
    - Use AVG for averages
    - Use GROUP BY only when explicitly required
